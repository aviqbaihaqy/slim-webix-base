<?php 
namespace App\Service\SortFilter;
...
class Parser
{
    ...
    /**
     * Get all col values of the table.
     *
     * @var mixed
     */
    ...
    public function __construct($builder, $params)
    {
        ...
        $this->params = $params;
        $this->builderFields = $builder->getConnection()->getSchemaBuilder()->getColumnListing($builder->getTable());
        $this->prefix = '_';
        $this->envelope = false;
    ...
    protected function parseFields($fieldsParam)
    {
	    ...
	    $fieldExist = $this->checkFieldExist($field);
	    if ($fieldExist) {
    ...
    protected function parseWith($withParam)
    {
        ...
        foreach (explode(',', $withParam) as $with) {
            ...
            for ($i = 0; $i <= $lastKey; $i++) {
                ...
                //Throw a new SortFilterException if the relation doesn't exist
                //or is not properly marked as a relation
                if (!$this->isRelation($previousModel, $part)) {
                    throw new Exception('UnknownResourceRelation', ['relation' => $part]);
                }
    ...
    protected function parseSort($sortParam)
    {
	    ...
	    $fieldExist = $this->checkFieldExist($sortElem);
		    if ($fieldExist) {
    ...
    protected function parseFilter($filterParams)
    {
	    ...
	    $fieldExist = $this->checkFieldExist($keyMatches[2]);
	    if ($fieldExist) {
