...
$app->group('/api', function () use ($container) {
    $this->get('', 'api.controller:root')->setName('root');
    $this->post('/register', 'security.auth.controller:register')->setName('api.register');
    $this->post('/login', 'security.auth.controller:login')->setName('api.login');
    $this->post('/auth/refresh', 'security.auth.controller:refresh')->setName('api.jwt.refresh');
    $this->get('/users/me', 'security.auth.controller:me')
        ->add($container['auth.middleware']())
        ->setName('api.users.me');
